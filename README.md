...
Собран на [Astro](https://astro.build) — статика, без UI-фреймворков и клиентских библиотек.
...
src/
  ...
  styles/             ← CSS по секциям, порядок импортов = порядок каскада
...
1. **Контакт для RSVP** — `RSVP_CONFIG`:
   ```ts
   export const RSVP_CONFIG = {
     type: 'whatsapp',        // 'whatsapp' | 'telegram' | 'phone'
     phone: '70000000000',    // номер без + и пробелов
     telegram: '',            // ник без @ (если type: 'telegram')
   };
   ```
   Текст ответа собирается автоматически из полей формы (имя, фамилия, «+1», напитки).
   Сейчас стоит номер-заглушка. Для Telegram текст копируется в буфер обмена —
   t.me не принимает готовое сообщение.

2. **Ссылка на чат праздника** — `CHAT_URL`. Пока пустая, кнопка показывает подсказку
   «добавим ближе к дате».

3. **Хэштег для фото** — в `RULES`, пункт «Фото» (сейчас `#____`).

4. **Срок ответа RSVP** — `RSVP_DEADLINE`.

5. **Точка на карте** (по желанию) — сейчас метка ставится по адресу. Для точной точки
